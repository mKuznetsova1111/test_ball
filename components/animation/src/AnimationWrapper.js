...
export default class AnimationWrapper extends PixiWrapper {
  ...
  initAnimation({animationContainer, postprocessing, label, frame, type, clip: clipName, transform}, data) {
    const animationData = global.window[animationContainer];
    const clip = AnimationWrapper.fromCache(clipName) || new animationData.lib[clipName];
    clip.name = clipName;
